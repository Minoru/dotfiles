...
myKeys = [  -- names of keys can be found in haskell-X11 package in files
          ...
          , ((mod1Mask .|. shiftMask, xK_Up),    spawn "urxvtc -fn 'xft:Hack:pixelsize=14:lang=ru' -rv +sb -e proxychains -f /home/minoru/.proxychains/ncmpcpp.conf ncmpcpp")

            -- toggle statusbar
          , ((mod1Mask .|. shiftMask, xK_b), sendMessage ToggleStruts)

            -- choose application to run
          , ((mod1Mask .|. shiftMask, xK_p), shellPrompt def { font = "xft:DejaVu Sans Mono:pixelsize=14:lang=ru" })
...
main = do
    ...
    xmonad $ ewmh $
      ...
      def
        ...
        , terminal = "urxvtc -fn 'xft:Hack:pixelsize=14:lang=ru' -rv +sb"
